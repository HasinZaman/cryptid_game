...
fn fragment(
    ...
    var uv = vec2<f32> (
        0.,// % 1.0,
        abs(in.world_position.y - in.position.y) % 1.0
    );

    let x = vec3<f32> (1.0, 0., 0.);
    let y = vec3<f32> (0., 1., 0.);
    let z = vec3<f32> (0., 0., 1.);

    let dot_x = abs(dot(in.world_normal, x));
    let dot_y = abs(dot(in.world_normal, y));
    let dot_z = abs(dot(in.world_normal, z));

    if dot_y > max(dot_x, dot_z) {
        return vec4<f32>(0.0, 0.0, 0.0, 1.);
    }
    
    // walls in in any direction
    // let u_dir = normalize(cross(in.world_normal, y));

    // // uv.x = sqrt(dot(
    // //     dot(
    // //         vec2<f32>(in.world_position.xz - in.position.xz),
    // //         u_dir.xz
    // //     ) * u_dir,
    // //     dot(
    // //         vec2<f32>(in.world_position.xz - in.position.xz),
    ...
    if (abs(dot(in.world_normal, x)) > abs(dot(in.world_normal, z))) {
        uv.x = abs(in.world_position.z - in.position.z) % 1.;
    }
    else {
        uv.x = abs(in.world_position.x - in.position.x) % 1.;
    }
    ...
    var output_color: vec4<f32> = vec4<f32>(uv.xy, 0., 0.75);//pbr_bindings::material.base_color;

    var pbr_input: pbr_functions::PbrInput;
    
    //pbr material
    pbr_input.material.base_color = output_color;// colour;
    ...
    pbr_input.material.flags = pbr_types::STANDARD_MATERIAL_FLAGS_ALPHA_MODE_MASK;;
